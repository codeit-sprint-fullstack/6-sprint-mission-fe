...
const Pagination = ({ pageLoad, params, totalCount }) => {
  ...
  return (
    <div className={style.pageBtnContainer}>
      <button onClick={decreasePage} className={style.pageBtn}>
        <img
          src="/assets/image/items/pagination/ic_arrow_left.svg"
          alt="왼쪽 화살표"
        />
      </button>
      {pages.map((page) => {
        ...
        return (
          <button
            ...
            className={`${style.pageBtn} ${active[page] ? style.active : ""}`}
          >
            {page}
          </button>
        );
      })}
      <button onClick={increasePage} className={style.pageBtn}>
        <img
          src="/assets/image/items/pagination/ic_arrow_right.svg"
          alt="오른쪽 화살표"
        />
      </button>
    </div>
